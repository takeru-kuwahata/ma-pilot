...
-- まず、admin@ma-pilot.local のユーザーIDを取得
-- （Supabase Authで既に作成済みと仮定）

-- 医院データ挿入（UUIDを明示的に指定）
INSERT INTO clinics (id, name, postal_code, address, phone_number, latitude, longitude, owner_id, is_active)
VALUES (
  'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'::uuid,  -- 固定UUID
  'さくら歯科クリニック',
  '150-0001',
  '東京都渋谷区神宮前1-1-1',
  '03-1234-5678',
  35.6762,
  139.7025,
  (SELECT id FROM auth.users WHERE email = 'admin@ma-pilot.local' LIMIT 1),
  true
)
...
VALUES
  (gen_random_uuid(), '診察券', 'card', 15000, 100, '標準的な診察券（100枚単位）'),
  (gen_random_uuid(), '名刺', 'card', 12000, 100, 'スタッフ用名刺（100枚単位）'),
  (gen_random_uuid(), 'リコールハガキ', 'postcard', 8000, 100, '定期検診案内ハガキ（100枚単位）')
...
-- 医院データ確認
SELECT id, name, postal_code, address FROM clinics;
