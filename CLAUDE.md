...
ドキュメント配置:
  ...
  アーカイブ（docs_archive/、ルート直下）:
    - 実装完了済みの詳細設計（コードが真実源となったもの）
    - Phase 6未実装機能の詳細設計（実装時に最新化して再作成）
    - 調査資料・事例研究（企画段階で使用、実装フェーズでは不要）
    - Git管理外（.gitignore）、Claude Code読み込み対象外（.claudeignore）

  アーカイブ（docs/archive/、docs配下）:
    - docs_archiveとは別物。Git管理下にあり、実装済み機能の過去ステータスレポート・
      修正完了報告・旧実装計画など「完了した作業の記録」を格納
    - status/配下: 機能別の実装完了時ステータス（現行はSCOPE_PROGRESS.mdに集約済み）
    - e2e-specs/, temp/: 完了したE2E設計・一時検証資料
