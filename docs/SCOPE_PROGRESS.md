...
|:-----:|------|:----:|
| 0 | 要件確認・設計書作成 | ✅ |
| 1 | DB変更（openhouse_status追加） | ⬜ |
| 2 | 型定義更新（types/index.ts） | ⬜ |
| 3 | バックエンド実装 | ⬜ |
| 4 | フロントエンド実装 | ⬜ |
| 5 | Stripe動作確認（テストモード） | ⬜ |
| 6 | 本番キーへ切り替え・デプロイ | ⬜ |
...
1. **機能3: 内覧会ステータス**（影響範囲最小、情報待ち不要）
2. **機能2: WordPress CSV一括移植**（WordPress管理者ユーザー名待ち）
3. **機能1: Lstep Webhook**（Lstep申し込み完了待ち + フォーム項目一覧待ち）
4. **機能4: Stripe決済**（Stripe APIキー待ち）
...
| ファイル | 用途 |
|---------|------|
| `backend/src/api/webhook.py` | Lstep・Stripe Webhook受信ルーター |
| `backend/src/services/stripe_service.py` | Stripe決済サービス |
| `frontend/src/services/stripe.ts` | Stripe初期化 |
| `supabase/migrations/20260324_add_openhouse_status.sql` | DBマイグレーション |

#### 修正ファイル
| ファイル | 変更内容 |
|---------|---------|
| `backend/src/core/config.py` | Stripe・WordPress・Lstep環境変数追加 |
| `backend/src/api/admin.py` | CSVインポート・ステータス変更エンドポイント追加 |
| `backend/src/api/print_orders.py` | Stripe決済エンドポイント追加 |
| `backend/src/models/clinic.py` | openhouse_statusフィールド追加 |
| `backend/main.py` | webhook.pyルーター登録 |
| `backend/requirements.txt` | stripe==11.5.0追加 |
| `backend/.env` | 環境変数追加（WordPress済み） |
| `frontend/src/types/index.ts` | OpenhouseStatus型・Clinic型拡張 |
| `frontend/src/pages/admin/AdminClinics.tsx` | ステータスフィルタ・ソート・CSVインポートUI追加 |
| `frontend/src/pages/PrintOrderForm.tsx` | Stripe Elements・決済フロー実装 |
| `frontend/package.json` | @stripe/stripe-js・@stripe/react-stripe-js追加 |

### 実装チェックリスト

#### Stage 1: DB変更
- [ ] `supabase/migrations/20260324_add_openhouse_status.sql` 作成
- [ ] Supabaseダッシュボードで開発環境に適用
- [ ] 本番環境に適用

#### Stage 2: 型定義
- [ ] `OpenhouseStatus` 型追加
- [ ] `Clinic` 型に `openhouse_status` フィールド追加
- [ ] `API_PATHS` にWebhook・Stripe・インポートのパス追加

#### Stage 3: バックエンド
| タスク | エンドポイント | メソッド | 実装 | 完了 |
|--------|--------------|---------|:----:|:----:|
| Lstep Webhook受信 | /api/webhooks/lstep | POST | [ ] | [ ] |
| Stripe Webhook受信 | /api/webhooks/stripe | POST | [ ] | [ ] |
| WPユーザー一括インポート | /api/admin/import-wordpress-users | POST | [ ] | [ ] |
| 内覧会ステータス変更 | /api/admin/clinics/{id}/openhouse-status | PUT | [ ] | [ ] |
| Stripe PaymentIntent作成 | /api/print-orders/{id}/create-payment-intent | POST | [ ] | [ ] |
| Stripe決済確定 | /api/print-orders/{id}/confirm-payment | POST | [ ] | [ ] |

#### Stage 4: フロントエンド
| タスク | 実装 |
|--------|:----:|
| AdminClinics: 内覧会ステータス列・フィルタ・ソート追加 | [ ] |
| AdminClinics: CSVインポートダイアログ追加 | [ ] |
| AdminClinics: パスワード管理UI追加 | [ ] |
| PrintOrderForm: Stripe Elements統合 | [ ] |
| PrintOrderForm: 決済完了フロー実装 | [ ] |

#### Stage 5: Stripe動作確認
- [ ] テストカード（4242 4242 4242 4242）で決済フロー確認
- [ ] Stripe Webhook受信確認（ngrok使用）
- [ ] payment_status更新確認
...
| # | 内容 | 担当 | 状態 |
|---|------|------|:----:|
| 1 | WordPress管理者ユーザー名 | クライアント | ⬜ 待ち |
| 2 | Application Password発行者のメールアドレス | クライアント | ⬜ 待ち |
| 3 | Stripeテスト用公開可能キー（pk_test_...） | クライアント | ⬜ 待ち |
| 4 | Stripeテスト用シークレットキー（sk_test_...） | クライアント | ⬜ 待ち |
| 5 | Stripe本番用公開可能キー（pk_live_...） | クライアント | ⬜ 待ち |
| 6 | Stripe本番用シークレットキー（sk_live_...） | クライアント | ⬜ 待ち |
| 7 | Lstep Webhook申し込み（主管理者） | クライアント | ⬜ 待ち |
| 8 | Lstepフォーム取得項目一覧 | クライアント | ⬜ 待ち |
