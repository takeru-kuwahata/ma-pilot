...
| # | 項目 | 手順 | 期待結果 | 結果 | 備考 |
|---|------|------|----------|------|------|
| 4.1.1 | ページ表示 | 左メニュー「医院設定」クリック | 医院情報フォームが表示 | ✅ | UI完成 |
...
**注**: ページUIは完成しているが、データの読み込み・保存機能が未実装（handleSaveBasicInfo/handleSaveBusinessInfoが空）。バックエンドAPIは実装済みのため、フロントエンド接続のみで完成（工数3時間）。詳細は`docs/CLINIC_SETTINGS_STATUS.md`参照。

### 4.2 スタッフ管理 (`/staff-management`)

| # | 項目 | 手順 | 期待結果 | 結果 | 備考 |
|---|------|------|----------|------|------|
| 4.2.1 | ページ表示 | 左メニュー「スタッフ管理」クリック | スタッフリストが表示 | ❌ | **フィールド名不一致でエラー** |
| 4.2.2 | スタッフ一覧表示 | テーブル確認 | スタッフが表示される | ❌ | clinic_id取得失敗 |
| 4.2.3 | スタッフ招待 | 「スタッフ招待」ボタンクリック | ダイアログが表示される | ❌ | **未実装**（alert） |
| 4.2.4 | スタッフ編集 | スタッフ編集ボタンクリック | 編集ダイアログが表示 | ❌ | **未実装**（alert） |
| 4.2.5 | スタッフ削除 | 削除ボタンクリック | 確認後削除される | ⚠️ | 実装済みだがテスト不可 |

**🔴 緊急問題**: フィールド名の不一致
- **原因**: バックエンド（Python）は`clinic_id`（スネークケース）、フロントエンド（TypeScript）の一部が`clinicId`（キャメルケース）
- **影響**: StaffManagement.tsx, DataManagement.tsx, Simulation.tsx等で「クリニック情報が見つかりません」エラー
- **修正方法**: 全フロントエンドページを`clinic_id`に統一（工数2時間）
- **詳細**: `docs/STAFF_CLINIC_SETTINGS_STATUS.md`参照
