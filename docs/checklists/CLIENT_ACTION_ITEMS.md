...
| 3 | レポート生成機能のテスト | 🔴 高 | 月次データ登録後 | ⏳ 未着手 |
| 4 | Google Maps APIキー取得・共有 | 🟡 中 | 1週間以内 | ⏳ 未着手 |
| 5 | 本番環境での動作確認（全ページ） | 🔴 高 | 全機能実装後 | ⏳ 未着手 |
...
#### ステップ1: レポート管理ページにアクセス
1. 左メニュー「レポート管理」をクリック

#### ステップ2: レポートテンプレートからダウンロード
3つのテンプレートカードが表示されています:
1. **月次経営レポート**: 売上、利益、患者数などの月次データレポート
2. **診療圏分析レポート**: 診療圏内の人口統計、競合分析レポート
3. **シミュレーション結果レポート**: 経営シミュレーションの結果レポート

各カードの右下にあるダウンロードボタン（↓アイコン）をクリックしてください。

#### ステップ3: レポート生成・ダウンロード確認
1. ダウンロードボタンをクリック
2. 「レポートを生成しています...」というローディング表示が出る
3. 数秒後、「レポートを生成しました」という成功メッセージが表示される
4. PDFファイルが自動的にダウンロードされる

#### ステップ4: レポート生成履歴の確認
1. ページ下部の「レポート生成履歴」テーブルを確認
2. 生成したレポートが一覧に表示される
3. ステータスが「完了」になっている
4. 履歴からもダウンロードボタンでPDFダウンロード可能

### ✅ 確認項目
以下の項目が正常に動作することを確認:
- ✅ 月次経営レポートのダウンロード
- ✅ 診療圏分析レポートのダウンロード
- ✅ シミュレーション結果レポートのダウンロード
- ✅ レポート生成履歴への反映
- ✅ 履歴からのダウンロード
