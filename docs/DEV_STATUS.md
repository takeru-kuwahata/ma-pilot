...
| L-5 | Lstep GAS「変更時」トリガー未適用の残り2フォーム（openhouse 710696・staff 710319）は、doctor_other（710762）と同じ「フォーム送信時トリガーが発火しない」不具合を将来的に起こす可能性がある。`docs/lstep-gas/` の修正版スクリプト・手順書を適用すれば解消 |
...
| 日付 | 内容 |
|------|------|
| 2026-07-13 | Lstep GASトリガー方式の恒久修正（doctor_otherフォーム、「フォーム送信時」→「変更時」）。パートナー企業の編集・削除機能追加、サービス保存ボタンの誤動作修正（詳細は SCOPE_PROGRESS.md 参照） |
| 2026-05-27 | クリニック側ページのモバイルレスポンシブ対応（診療圏分析・レポート・スタッフ管理・医院設定） |
| 2026-05-27 | セッション切れ後の再ログインで元のページに戻る機能追加 |
| 2026-05-27 | ダッシュボードに前年同月比・固定費・変動費率KPI追加、ユニット稼働率→変動費率グラフに変更 |
| 2026-05-27 | 診療圏分析: 競合密度を「1院あたり人口（人/院）」表記に変更、自院ピン廃止 |
| 2026-05-27 | スタッフ招待メール: Supabase SMTP非依存化（generateLink + Resend直接送信）、クリニック名入り |
| 2026-05-26 | メール送信をResend APIに全面切り替え（SMTP廃止） |
| 2026-05-25 | フロントエンドサービステスト強化・Lstep Webhookユニットテスト実装（計+59テスト） |
| 2026-05-24 | 本番運用診断フェーズ1〜3: セキュリティ・パフォーマンス・テストカバレッジ改善 |
| 2026-05-20 | SMTPリレー切り替え（Google Workspace IPアドレス認証） |
