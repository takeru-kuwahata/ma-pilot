# Lステップ連動 経営管理・戦略シミュレーション機能 - 要件定義書
...
開業歯科医院がLステップ経由で経営数値を入力・自動取得し、リアルタイムな可視化・目標達成シミュレーション・戦略レポート生成を実現。データ駆動の経営判断と有料プランへの自然な転換を促す。
...
3. **Lステップ連携成功率**: 95%以上のユーザーでLステップからのCSVデータ取込が正常動作
...
2. **入力の手軽さ**: 「Lステップデータが自動で入り、追加入力も迷わずできた」と評価
...
- **Lステップ連携**: CSVデータ取込によるLステップ顧客データ活用
...
4. Lステップデータの場合: CSV形式チェック、新患数・リコール率等の自動抽出
5. 保存成功時、ダッシュボードに反映
...
- **Lステップ**: MVP版では手動CSVエクスポート運用（完全自動化には月額5,500円のWebhook転送機能が必要）
...
| サービス名 | 用途 | 取得先 | 備考 |
|-----------|------|--------|------|
| Lステップ Webhook転送機能 | リアルタイム顧客データ連携 | Lステップ管理画面 | 月額5,500円 |
...
### Phase 2: 自動化・高度化
- Lステップ Webhook転送機能導入（リアルタイム自動連携）
